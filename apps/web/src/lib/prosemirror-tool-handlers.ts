...
export const applyHeading = (editorView: EditorView, level: number): void => {
  ...
  if (!nodeType) {
    console.warn(
      `Schema missing '${level === 0 ? "paragraph" : "heading"}' node type.`
    );
    return;
  }
...
export const insertLink = (
  ...
  if (resolvedFrom === resolvedTo) {
    ...
    console.log(
      `InsertLink: Replacing selection from ${resolvedFrom} to ${resolvedTo}`
    );
...
export const toggleList = (
  ...
  const { schema } = state;
  const listNodeType = schema.nodes[listTypeName];
  const listItemType = schema.nodes.list_item;

  if (!listNodeType || !listItemType) {
    throw new Error("List node schema or list node type not present!!!");
  }

  const alreadyWithinList = isSelectionInSpecificListType(
    state,
    listNodeType,
    listItemType
  );
  console.log("Already within list: ", alreadyWithinList);

  if (alreadyWithinList) {
  ...
  if (isHeading) {
    ...
    if (headingNode) {
      const paragraphNode = paragraphType.create(null, headingNode.content);
      tr.replaceWith(pos, pos + headingNode.nodeSize, paragraphNode);
      dispatch(tr);

      const command = wrapInList(listNodeType);
...
function isSelectionInSpecificListType(
  state: EditorState,
  targetListType: NodeType,
  listItemType: NodeType | undefined
): boolean | void {
  if (!listItemType) {
    throw new Error("List item type is either not defined!");
  }
  ...
  for (let depth = currentDepth; depth >= 0; depth--) {
    const currentNode: Node = $pos.node(depth);
    if (currentNode.type === listItemType) {
      const immediateParent: Node = $pos.node(depth - 1);
      if (immediateParent.type === targetListType) {
        return true;
      }
    }
  }
